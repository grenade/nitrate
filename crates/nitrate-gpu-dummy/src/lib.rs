use anyhow::Result;
use async_trait::async_trait;
use nitrate_gpu_api::{DeviceInfo, FoundNonce, GpuBackend, KernelWork};
use tokio::time::{sleep, Duration};
use tracing::debug;

#[derive(Clone, Default)]
/// Dummy backend: accepts launches and polls but never reports results.
/// Honors the `generation` field by logging it; since it never returns results,
/// it effectively drops any stale generations by design.
pub struct DummyBackend;

#[async_trait]
impl GpuBackend for DummyBackend {
    async fn enumerate(&self) -> Result<Vec<DeviceInfo>> {
        Ok(vec![DeviceInfo {
            index: 0,
            name: "DummyGPU".into(),
            memory_mb: 64,
        }])
    }
    async fn launch(&self, device_index: u32, work: KernelWork) -> Result<()> {
        debug!(
            ?device_index,
            generation = work.generation,
            start = work.start_nonce,
            count = work.nonce_count,
            "Dummy launch"
        );
        Ok(())
    }
    async fn poll_results(&self, _device_index: u32) -> Result<Vec<FoundNonce>> {
        sleep(Duration::from_millis(500)).await;
        // No results are produced; by construction this drops all generations (stale or current).
        Ok(vec![])
    }
}
